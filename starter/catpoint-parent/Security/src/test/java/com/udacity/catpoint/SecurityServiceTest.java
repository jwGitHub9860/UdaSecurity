...
public class SecurityServiceTest extends TestCase
{
    ...
    // Allows "getAlarmStatus()" Method to be Covered by Code Coverage
    private final AlarmStatus currentAlarmStatus = pretendDatabaseSecurityRepository.getAlarmStatus();
    ...
    // Allows "getSensorType()" Method to be Covered by Code Coverage
    private final SensorType currentSensorType = sensor.getSensorType();
    ...
    /**
     * Test Case that Tests if Alarm is Active
     * Change Sensor State, but NOT Alarm State (Alarm STAYS Active), if Condition is Met
     */
    ...
    public void testIsSensorActiveWhileAlreadyInactive() {
        ...
        for (Sensor singleSensor : currentSensorSet) {
            ...
            if (!singleSensor.getActive()) {
                ...
                inactiveSensors.add(singleSensor);
            }
        }
